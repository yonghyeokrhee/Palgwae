...
def build_parser() -> argparse.ArgumentParser:
    ...
    bootstrap = commands.add_parser(
        "bootstrap",
        help="initialize a project bundle from a conventional spec or safe starter",
    )
    bootstrap.add_argument(
        "--project-root", type=Path, default=Path(".")
    )
    bootstrap.add_argument(
        "--output", type=Path, default=Path(".palgwae/bundle")
    )
    bootstrap.add_argument("--spec", type=Path)
    ...
    mcp.add_argument(
        "--bootstrap",
        action="store_true",
        help="initialize a missing bundle from the current project before serving",
    )
    mcp.add_argument(
        "--spec",
        type=Path,
        help="project spec used only when --bootstrap must create the bundle",
    )
    mcp.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "mcp":
            if args.bootstrap:
                bootstrap_project(Path.cwd(), args.bundle, spec=args.spec)
        ...
        _print_json(result)
        return 0
    except (BundleValidationError, KeyError, RuntimeError, ValueError) as exc:
